...
def ban_user(db, id, reason):
    ...
    db.modify(
        "UPDATE users SET rank = ?, left = ?, blacklistReason = ? WHERE id = ?", param)
    return 1, 0


def unban_user(db, id):
    c = db.execute(
        "SELECT left FROM users WHERE id = ? AND rank = ?", (id, -10))
    row = c.fetchone()
    if row is None:
        return 0
    if row[0] == datetime.utcfromtimestamp(0):
        ...
        db.modify(
            "UPDATE users SET rank = ?, blacklistReason = NULL WHERE id = ?", (0, id))
    return 1
...
def sync(d):
    ...
    while True:
        ...
        for name, db in d.items():
            c = db.execute(
                "SELECT id, blacklistReason FROM users WHERE rank = ? AND left >= ?", (-10, last_update))
            for row in c:
                reason = row[1] or ""
        ...
        for id, reason, from_name in l:
            ...
            if stat1 + stat2 > 0:
                logging.info(
                    "Transferred ban of user id %d orginated from %s (b:%d p:%d)", id, from_name, stat1, stat2)
        # Zzz..
        last_update = now
        sleep(interval)
...
def main(argv):
    ...
    actions = {
        "blacklist": c_blacklist, "unblacklist": c_unblacklist, "find": c_find, "sync": c_sync
    }
